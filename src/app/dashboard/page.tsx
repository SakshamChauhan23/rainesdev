...
export default async function DashboardPage({
  ...
  const totalRevenue = agents.reduce(
    (acc, agent) => acc + agent.purchaseCount * Number(agent.price),
    0
  )
    ...
    <div className="min-h-screen bg-brand-cream">
        ...
        <Container className="py-12">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/submit-agent">
              <Button className="h-12 rounded-xl bg-brand-orange font-semibold text-white shadow-lg shadow-brand-orange/30 transition-all hover:-translate-y-0.5 hover:bg-brand-orange/90 hover:shadow-xl hover:shadow-brand-orange/40">
      ...
      <Container className="py-12">
        ...
        {searchParams?.success === 'created' && (
          <div className="to-brand-orange/10/50 mb-8 animate-fade-in rounded-3xl border-2 border-brand-orange/20 bg-gradient-to-br from-brand-orange/5 p-6 shadow-lg">
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-orange/80 to-brand-orange shadow-lg shadow-brand-orange/30">
                <CheckCircle className="h-6 w-6 text-white" />
              </div>
              <div>
                <p className="mb-1 text-lg font-semibold text-brand-slate">
          ...
          {agents.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
              {agents.map(agent => (
                <div
                  key={agent.id}
                  className="group relative overflow-hidden rounded-2xl border-2 border-brand-slate/10 bg-white shadow-lg transition-all hover:-translate-y-1 hover:border-brand-orange/30 hover:shadow-xl sm:rounded-3xl"
                >
                  {/* Thumbnail */}
                  {agent.thumbnailUrl && (
                    <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-brand-orange/5 to-brand-orange/5">
                      <img
                  ...
                  <div className="p-4 sm:p-6">
                    ...
                    <div className="mb-3 flex flex-wrap items-center gap-1.5 sm:mb-4 sm:gap-2">
                      ...
                      <Badge className="ml-auto rounded-lg border-brand-orange/20 bg-brand-orange/10 text-xs font-semibold text-brand-orange sm:text-sm">
                        {formatPrice(Number(agent.price))}
                      </Badge>
                    </div>

                    {/* Title & Description */}
                    <h3 className="mb-2 line-clamp-1 text-lg font-bold text-brand-slate sm:text-xl">
                      {agent.title}
                    </h3>
                    <p className="mb-3 line-clamp-2 text-sm text-brand-slate/60 sm:mb-4">
                      {agent.shortDescription}
                    </p>

                    {/* Update Warning */}
                    {agent.status === 'APPROVED' && agent.hasActiveUpdate && (
                      <div className="mb-4 flex items-start gap-2 rounded-2xl border-2 border-amber-200 bg-amber-50 p-3">
                    ...
                    <div className="mb-4 flex items-center gap-3 rounded-xl bg-brand-cream p-3 sm:mb-6 sm:gap-4 sm:rounded-2xl sm:p-4">
                      <div className="flex-1 text-center">
                        <p className="text-xl font-bold text-brand-slate sm:text-2xl">
                          {agent.viewCount}
                        </p>
                    ...
                    <div className="flex gap-2 border-t-2 border-brand-slate/10 pt-3 sm:pt-4">
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 rounded-xl border-2 hover:border-brand-orange hover:bg-brand-orange/5"
                        asChild
                      >
                      ...
                      {agent.status === 'DRAFT' && (
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/dashboard/agents/${agent.id}/edit`}>Edit & Submit</Link>
                        </Button>
                      )}
                      {agent.status === 'REJECTED' && (
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/dashboard/agents/${agent.id}/edit`}>Fix & Resubmit</Link>
                        </Button>
                      )}
                      {agent.status === 'UNDER_REVIEW' && (
                        <Button
                          size="sm"
                          className="flex-1 rounded-xl bg-brand-slate/20 text-brand-slate"
                          disabled
                        >
                          Under Review
                        </Button>
                      ...
                      {agent.status === 'APPROVED' && !agent.hasActiveUpdate && (
                        <RequestUpdateButton agentId={agent.id} agentTitle={agent.title} />
                      )}
                      {agent.status === 'APPROVED' && agent.hasActiveUpdate && (
                        <Button
                          size="sm"
