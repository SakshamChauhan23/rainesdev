 ...
 */
export async function hasPurchasedVersion(
  buyerId: string,
...
/**
 * Check if a buyer has any completed purchases (for legacy user detection)
 */
export async function hasAnyCompletedPurchase(userId: string): Promise<boolean> {
  try {
    ...
    logger.error('Error checking completed purchases:', error)
    return false
  }
}
