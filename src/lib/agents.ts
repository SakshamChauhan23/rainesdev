...
// Cached version of getAgentBySlug - revalidates every 30 seconds
export const getCachedAgentBySlug = unstable_cache(
  async (slug: string) => {
...
// Get all agent slugs for static generation
export async function getAllAgentSlugs() {
  try {
    ...
    logger.error('Error fetching agent slugs:', error)
    return []
  }
}
