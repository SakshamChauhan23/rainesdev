...
import { ShoppingCart, Store, Search, CreditCard, Settings, TrendingUp, BarChart3, Users, MessageSquare, Shield, Zap, CheckCircle2 } from 'lucide-react'
  ...
  const buyerFeatures = [
    {
      ...
      description: 'Browse our curated marketplace of ready-to-deploy AI agents for every business need',
      color: 'orange'
    },
    {
      icon: Shield,
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-white py-20 sm:py-28 lg:py-32">
      ...
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/4 left-0 h-96 w-96 rounded-full bg-brand-teal/5 blur-3xl" />
      ...
      <Container>
        ...
        {/* Tab Switcher */}
        <div className={`mx-auto mb-12 flex w-fit rounded-2xl bg-brand-cream p-2 shadow-lg transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          <button
          ...
          >
            <ShoppingCart className={`mb-1 inline-block h-5 w-5 transition-transform ${activeTab === 'buyer' ? 'scale-110' : 'group-hover:scale-110'}`} />
            <span className="ml-2">For Buyers</span>
            {activeTab === 'buyer' && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-brand-orange rounded-full" />
            )}
          </button>
          <button
            ...
            className={`group relative rounded-xl px-8 py-4 text-sm font-semibold transition-all duration-300 ${
              activeTab === 'seller'
                ...
                : 'text-brand-slate/60 hover:text-brand-teal'
            }`}
          >
            <Store className={`mb-1 inline-block h-5 w-5 transition-transform ${activeTab === 'seller' ? 'scale-110' : 'group-hover:scale-110'}`} />
            <span className="ml-2">For Sellers</span>
            {activeTab === 'seller' && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-brand-teal rounded-full" />
            )}
          </button>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={`${activeTab}-${index}`}
                className={`group rounded-3xl bg-brand-cream/50 p-8 backdrop-blur-sm border border-brand-slate/5 transition-all duration-500 hover:bg-white hover:shadow-xl hover:-translate-y-2 ${
                  isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
                style={{ transitionDelay: `${300 + index * 100}ms` }}
              >
                <div className={`mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110 ${
                  isOrange
                ...
                <p className="text-brand-slate/70 leading-relaxed">{feature.description}</p>
              </div>
            )
          })}
        </div>
